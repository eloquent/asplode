...
class ExceptionHandlerStackTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        Phunky::when($this->isolator)->set_exception_handler(Phunky::anyParameters())->thenReturn('foo');

        $this->assertSame('foo', $this->stack->handler());
        $restoreVerification = Phunky::verify($this->isolator, Phunky::times(2))->restore_exception_handler();
        Phunky::inOrder(
            Phunky::verify($this->isolator)->set_exception_handler(function () {}),
            $restoreVerification,
            $restoreVerification,
            Phunky::verify($this->isolator)->set_exception_handler('foo')
        );
    }

    public function testHandlerEmptyStack()
    {
        Phunky::when($this->isolator)->set_exception_handler(Phunky::anyParameters())->thenReturn(null);

        $this->assertNull($this->stack->handler());
        Phunky::inOrder(
            Phunky::verify($this->isolator)->set_exception_handler(function () {}),
            Phunky::verify($this->isolator)->restore_exception_handler()
        );
        Phunky::verify($this->isolator, Phunky::never())->set_exception_handler(null);
    }

    public function testHandlerStack()
    {
        Phunky::when($this->isolator)
            ->set_exception_handler(Phunky::anyParameters())
            ->thenReturn('foo')
            ->thenReturn('bar')
            ->thenReturn(null);

        $this->assertSame(array('foo', 'bar'), $this->stack->handlerStack());
        $setEmptyVerification = Phunky::verify($this->isolator, Phunky::times(3))->set_exception_handler(function () {});
        $restoreVerification = Phunky::verify($this->isolator, Phunky::times(5))->restore_exception_handler();
        Phunky::inOrder(
            ...
            $restoreVerification,
            Phunky::verify($this->isolator)->set_exception_handler('foo'),
            Phunky::verify($this->isolator)->set_exception_handler('bar')
        );
    }
    ...
    public function testPush()
    {
        ...
        Phunky::verify($this->isolator)->set_exception_handler('foo');
        Phunky::verify($this->isolator, Phunky::never())->restore_exception_handler();
    }
    ...
    public function testRestore()
    {
        ...
        $setEmptyVerification = Phunky::verify($this->isolator, Phunky::times(2))->set_exception_handler(function () {});
        $restoreVerification = Phunky::verify($this->isolator, Phunky::times(3))->restore_exception_handler();
        Phunky::inOrder(
            $setEmptyVerification,
            $restoreVerification,
            $restoreVerification,
            ...
            $restoreVerification,
            Phunky::verify($this->isolator)->set_exception_handler('bar'),
            Phunky::verify($this->isolator)->set_exception_handler('baz')
        );
    }
    ...
    public function testExecuteWith()
    {
        ...
        $setEmptyVerification = Phunky::verify($this->isolator, Phunky::times(4))->set_exception_handler(function () {});
        $restoreVerification = Phunky::verify($this->isolator, Phunky::times(6))->restore_exception_handler();
        Phunky::inOrder(
            $setEmptyVerification,
            $restoreVerification,
            $restoreVerification,
